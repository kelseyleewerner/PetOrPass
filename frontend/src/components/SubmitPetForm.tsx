...
export function SubmitPetForm(): JSX.Element {
  ...
  const onSubmitPet: (
    ...
    if (user.success) {
      ...
      const uri = `http://${import.meta.env.VITE_BACKEND_IP}:${
  ...
  return (
    <form className="container below-navbar">
      ...
      <div className="row mt-3 justify-content-center">
        <div className="col-xl-4 col-md-6 col-sm-8 col-10">
          <label className="form-label" htmlFor="petImageInput">
            Upload Pet Image
          </label>
          <input
            className="form-control"
...
function SubmitSuccessMessage(): JSX.Element {
  return (
    <div className="row text-center mt-3 success-text">
      <p>
        <strong>Success! Thanks for submitting your pet &#60;3</strong>
      </p>
    </div>
  );
}
...
function SubmitErrorMessage(): JSX.Element {
  return (
    <div className="row text-center mt-3 error-text">
      <p>
        <strong>
          uh oh! Looks like something went wrong! Try re-submitting with a
          different pet name and/or image
        </strong>
      </p>
    </div>
  );
